...
func handleConnection(conn net.Conn, serverEdPriv ed25519.PrivateKey) {
	...
	// 2. Создаем TUN адаптер для СЕРВЕРА
	// Внимание: Имя адаптера "VPNServer", чтобы не конфликтовать с клиентом
	adapter, err := wintun.CreateAdapter("VPNServer", "Server", nil)
	if err != nil {
		log.Printf("❌ Ошибка создания TUN на сервере: %v", err)
		return
	}
	defer adapter.Close()
	...
	if err != nil {
		log.Printf("❌ Ошибка старта сессии: %v", err)
		return
	}
	defer session.End()

	fmt.Println("✅ SERVER TUN UP! Waiting for IP config...")
	fmt.Println("❗ ВТОРОЕ ОКНО: netsh interface ip set address name=\"VPNServer\" source=static addr=10.1.0.1 mask=255.255.255.0 gateway=none")

	time.Sleep(5 * time.Second)
	fmt.Println("🚀 TUNNEL READY! Listening for packets...")
	...
	go func() {
		...
		for {
			...
			// Г. Пишем в TUN (ОС сервера получает пакет)
			// Windows увидит пакет от 10.1.0.2, адресованный 10.1.0.1
			packet, err := session.AllocateSendPacket(len(decrypted))
			if err == nil {
				copy(packet, decrypted)
				session.SendPacket(packet)
				// fmt.Printf("⬇ Inbound: %d bytes\n", len(decrypted)) // Раскомментируй для отладки
	...
	go func() {
		...
		for {
			...
			if err != nil {
				// Мелкие ошибки драйвера игнорируем
				continue
			}
			...
			// В. Готовим пакет для TCP: [Длина] + [Encrypted]
			...
			// fmt.Printf("⬆ Outbound: %d bytes\n", len(finalPkg)) // Раскомментируй для отладки
		}
	}()
